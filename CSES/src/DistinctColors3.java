...
public class DistinctColors3 {
    private static final ArrayList<ArrayList<Integer>> adj = new ArrayList<>();
    private static ArrayList<HashSet<Integer>> col;
        ...
        for (int next : adj.get(at)) {
            if (next != prev) {
                ...
                if (col.get(at).size() < col.get(next).size()) {
                    HashSet<Integer> temp = new HashSet<>(col.get(at));
                    col.set(at, col.get(next));
                    col.set(next, temp);
                }
                for (int c : col.get(next)) {
                    col.get(at).add(c);
                }
                col.get(next).clear();
            }
