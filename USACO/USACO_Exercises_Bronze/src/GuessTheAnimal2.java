...
public class GuessTheAnimal2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<GuessTheAnimal2> characteristicsList = new ArrayList<>();
        boolean has;
        ...
        for (int i = 0; i < numAnimals; i++){
            for (int j = 2; j <= (1 + Integer.parseInt(data[i][1])); j++){
                has = false;
                for (GuessTheAnimal2 guessTheAnimal : characteristicsList) {
                ...
                if (!has){
                    GuessTheAnimal2 temp = new GuessTheAnimal2(1, data[i][j]);
        ...
        for (String[] strings : temp) {
            characteristicsList.add(new GuessTheAnimal2(Integer.parseInt(strings[0]), strings[1]));
        }
//
...
//        }
        ...
        for (GuessTheAnimal2 guessTheAnimal2 : characteristicsList) {
            String[][] temp2 = new String[numAnimals][102];
            for (int j = 0; j < numAnimals; j++) {
                boolean contains = false;
                for (int k = 2; k < 102; k++) {
                    if (finalList[j][k] != null) {
                        if (finalList[j][k].equals(guessTheAnimal2.characteristic)) {
                            contains = true;
                            break;
                        }
                    } else {
    ...
    private GuessTheAnimal2(int o, String c){
        occurrences = o;
        characteristic = c;
    }
...
go through, crossing off each characteristic in order, until only one animal is left
*/