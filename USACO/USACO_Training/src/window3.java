...
public class window3 {
    ...
    private window3(int I, int x, int y, int X, int Y) {
        this.I = I;
        if (x < X) {
            left = x;
            right = X;
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<window3> windows = new ArrayList<>(); //bottom to top order (so index 0 is bottom, index x is x from bottom)
        ...
        while (line != null) {
            if (line.charAt(0) == 'w') {
                ...
                int Y = Integer.parseInt(line.substring(start, end));
                window3 window = new window3(I, x, y, X, Y);
                windows.add(window);
            } else if (line.charAt(0) == 't') {
                for (int i = 0; i < windows.size(); i++) {
                    if (windows.get(i).I == line.charAt(2)) {
                        window3 temp = windows.get(i);
                        windows.remove(i);
                        windows.add(temp);
                        break;
                    }
    ...
    private static double helper(ArrayList<window3> windows, int I) {
        int index = -1;
        for (int i = 0; i < windows.size(); i++) {
            window3 window = windows.get(i);
            if (window.I == I) {
                index = i;
//                total = Math.abs((window.right - window.left) * (window.top - window.bottom));
        ...
        ArrayList<window3> onTop = new ArrayList<>();
