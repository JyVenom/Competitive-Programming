...
public class cowtour2 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            loc[i][0] = Double.parseDouble(line[0]);
            loc[i][1] = Double.parseDouble(line[1]);
        }
        DijkstrasShortestPathAdjacencyListWithDHeap dij = new DijkstrasShortestPathAdjacencyListWithDHeap(n);
        for (int i = 0; i < n; i++) {
            String line = br.readLine();
            for (int j = 0; j < n; j++) {
                if (line.charAt(j) == '1') {
                    double a = Math.abs(loc[i][0] - loc[j][0]);
                    double b = Math.abs(loc[i][1] - loc[j][1]);
                    dij.addEdge(i, j, Math.sqrt(a * a + b * b));
                }
            }
        }
        br.close();

        double[] temp = dij.dijkstra(0);
        ...
        for (Integer integer : a) {
            for (Integer value : b) {
                double x = Math.abs(loc[integer][0] - loc[value][0]);
                double y = Math.abs(loc[integer][1] - loc[value][1]);
                double dist = Math.sqrt(x * x + y * y);
                dij.addEdge(integer, value, dist);
                dij.addEdge(value, integer, dist);
                double max = 0;
                for (int i = 0; i < n; i++) {
                    double[] all = dij.dijkstra(i);
                    for (int j = 0; j < n; j++) {
                        if (all[j] > max) {
                            max = all[j];
                        }
                ...
                //try all diameters and keep the largest one.
                //every time you are done finding the max, update the minimum max (min)
                dij.removeEdge(integer);
                dij.removeEdge(value);
            }
        }
        DecimalFormat df = new DecimalFormat("#0.000000");
        pw.println(df.format(min));
        pw.close();
    }

    public static class DijkstrasShortestPathAdjacencyListWithDHeap {
        ...
        public void removeEdge (int from) {
            graph.get(from).remove(graph.get(from).size() - 1);
        }
