...
public class window8 {
    ...
    private window8(int I, int x, int y, int X, int Y) {
        this.I = I;
        if (x < X) {
            left = x;
            right = X;
    ...
    public static void main(String[] args) throws IOException {
        ...
        window8[] windows = new window8[75];
        String line = br.readLine();
        while (line != null) {
            if (line.charAt(0) == 'w') {
                int I = line.charAt(2) - 48;
                int start = 4;
                ...
                int Y = Integer.parseInt(line.substring(start, end));
                windows[I] = new window8(I, x, y, X, Y);
                order.add(I);
            } else if (line.charAt(0) == 't') {
                int c = line.charAt(2) - 48;
                ...
                pw.println(df.format(helper(order, windows, line.charAt(2) - 48) * 100));
            }
    ...
    private static double helper(ArrayList<Integer> order, window8[] windows, int I) {
        int index = -1;
        for (int i = 0; i < order.size(); i++) {
            if (order.get(i) == I) {
                index = i;
        ...
        ArrayList<window8> onTop = new ArrayList<>();
        for (int i = index + 1; i < order.size(); i++) {
            onTop.add(windows[order.get(i)]);
        }
        window8 curWindow = windows[order.get(index)];
