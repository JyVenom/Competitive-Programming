...
public class lgame3 {
    static int[] letters = new int[256];
    private static final int[] scores = new int[256];
    static byte[] dict_bytes;
    static byte[] sort_bytes;
    static byte[] inputs;
    static int min_word_size = Integer.MAX_VALUE;
    static HashMap<Key, Word> wordMap = new HashMap<>();
    static int max_score = 0;
    static HashSet<String> max_set = new HashSet<>();
    static Key[] keys;
    ...
    static void build_result(ArrayList<String> list, int index, int words) {
        if (index >= words) {
            if (list.size() == 0)
                return;
    ...
    static void init_word(int start, int end) {
        ...
        Word word = wordMap.computeIfAbsent(key, k -> (Word) k);
