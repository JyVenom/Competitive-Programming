...
public class buylow3 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i <= n - max; i++) {
            int[] cur = new int[max];
            cur[0] = prices[i];
            findNum(next, sequences, cur, prices, max, 1, i, 1);
        }
    ...
    private static void findNum(ArrayList<ArrayList<Integer>> next, ArrayList<int[]> sequences, int[] cur, int[] prices, int max, int sum, int at, int at2) {
        if (sum == max) {
            sequences.add(cur.clone());
        } else {
            for (int temp : next.get(at)) {
                cur[at2] = prices[temp];
                findNum(next, sequences, cur, prices, max, sum + 1, temp, at2 + 1);
            }
        }
    }
