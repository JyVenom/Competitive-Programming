...
public class window5 {
    ...
    private window5(int I, int x, int y, int X, int Y) {
        this.I = I;
        if (x < X) {
            left = x;
            right = X;
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<window5> windows = new ArrayList<>(); //bottom to top order (so index 0 is bottom, index x is x from bottom)
        ...
        while (line != null) {
            if (line.charAt(0) == 'w') {
                ...
                int Y = Integer.parseInt(line.substring(start, end));
                window5 window = new window5(I, x, y, X, Y);
                windows.add(window);
            } else if (line.charAt(0) == 't') {
                for (int i = 0; i < windows.size(); i++) {
                    if (windows.get(i).I == line.charAt(2)) {
                        window5 temp = windows.get(i);
                        windows.remove(i);
                        windows.add(temp);
                        break;
                    }
    ...
    private static double helper(ArrayList<window5> windows, int I) {
        int index = -1;
        for (int i = 0; i < windows.size(); i++) {
            window5 window = windows.get(i);
            if (window.I == I) {
                index = i;
//                total = Math.abs((window.right - window.left) * (window.top - window.bottom));
        ...
        ArrayList<window5> onTop = new ArrayList<>();
                ...
                if (window.bottom < windows.get(index).top) {
                    if (window.right > windows.get(index).left) {
                        if (window.left < windows.get(index).right) {
                            ...
                            if (skip.get(top).isEmpty()) {
                                skip.set(top, new ArrayList<>(skip.get(parent[top])));
                            }
