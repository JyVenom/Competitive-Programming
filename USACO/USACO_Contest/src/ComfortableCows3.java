...
public class ComfortableCows3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < N; j++) {
                System.arraycopy(map[j], 0, map2[j], 0, N);
            }
            boolean[][] visited = new boolean[N][N];
            for (int j = 0; j < tmp.size(); j++) {
                visited[tmp.get(j)[0]][tmp.get(j)[1]] = true;
                int[] tmp2 = findMiss(map2, tmp.get(j)[0], tmp.get(j)[1], n);
                add.add(tmp2);
                map2[tmp2[0]][tmp2[1]] = true;
                cur++;
                for (int k = 0; k < 4; k++) {
                    int newRow = row + dirRow[k];
                    int newCol = col + dirCol[k];
                    if (isValid(newRow, newCol, n) && map2[newRow][newCol]) {
                        if (comf(map2, newRow, newCol, n) && !visited[newRow][newCol]) {
                            tmp.add(new int[]{newRow, newCol});
                        }
                    }
                }
...
//
    private static int[] findMiss(boolean[][] map, int row, int col, int n) {
        for (int i = 0; i < 4; i++) {
            int newRow = row + dirRow[i];
            int newCol = col + dirCol[i];
            if (!isValid(newRow, newCol, n) || !map[newRow][newCol]) {
