...
public class WhyDidTheCowCrossTheRoadIII {
    ...
    private static void findComponents(int[][] roads, int[][] all, int cur, int row, int col, int n) {
        all[row][col] = cur;
        if (col > 0 && all[row][col - 1] == 0 && binSearch(roads, 0, roads.length - 1, row * n + col, row * n + col - 1) == -1) { //left
            findComponents(roads, all, cur, row, col - 1, n);
        }
        if (col < n - 1 && all[row][col + 1] == 0 && binSearch(roads, 0, roads.length - 1, row * n + col, row * n + col + 1) == -1) { //right
            findComponents(roads, all, cur, row, col + 1, n);
        }
        if (row > 0 && all[row - 1][col] == 0 && binSearch(roads, 0, roads.length - 1, row * n + col, (row - 1) * n + col) == -1) { //up
            findComponents(roads, all, cur, row - 1, col, n);
        }
        if (row < n - 1 && all[row + 1][col] == 0 && binSearch(roads, 0, roads.length - 1, row * n + col, (row + 1) * n + col) == -1) { //down
            findComponents(roads, all, cur, row + 1, col, n);
        }
    }
