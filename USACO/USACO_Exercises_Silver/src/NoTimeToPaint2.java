...
public class NoTimeToPaint2 {
    ...
    private static int findCost(ArrayList<Integer> colors, ArrayList<Integer> changes) {
        int count = 1;
        for (int i = 1; i < colors.size(); i++) {
            int start = 0;
            for (int j = 0; j < changes.size(); j++) {
                if (changes.get(j).equals(colors.get(i))) {
                    start = j + 1;
                    break;
                }
            }
            ...
            for (int j = start; j < changes.size(); j++) {
                if (changes.get(j) < colors.get(i)) {
                    for (int k = j + 1; k < changes.size(); k++) {
                        if (changes.get(k).equals(colors.get(i))) {
                            j = k;
                            count++;
                            break;
                        }
