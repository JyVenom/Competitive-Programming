...
public class MilkVisits {
    public static void main(String[] args) throws IOException {
        ...
        DijkstrasShortestPathAdjacencyListWithDHeap barns = new DijkstrasShortestPathAdjacencyListWithDHeap(n);
        ...
        for (int i = 0; i < m; i++){
            st = new StringTokenizer(br.readLine());
            paths[i][0] = Integer.parseInt(st.nextToken()) - 1;
            paths[i][1] = Integer.parseInt(st.nextToken()) - 1;
            preferences[i] = st.nextToken().charAt(0);
        }

        for (int i = 0; i < m; i++){
            ArrayList<Integer> path = barns.reconstructPath(paths[i][0], paths[i][1]);
            boolean good = false;
            for (Integer barn : path) {
                if (breeds.charAt(barn) == preferences[i]) {
                    pw.print(1);
                    good = true;
                    break;
                }
