    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < prev.size(); j++) {
                state temp = new state(prev.get(j).x, prev.get(j).y);
                temp.x = x;
                if (!cur.contains(temp)) {
                    cur.add(temp);
                }

                temp = new state(prev.get(j).x, prev.get(j).y);
                temp.y = y;
                if (!cur.contains(temp)) {
                    cur.add(temp);
                }
        ...
        for (int i = 0; i < prev.size(); i++) {
            all.add(prev.get(i).x + prev.get(i).y);
        }
        ...
        if (ans >= 0) {
            ...
            int temp = -1 * (ans + 1);
            if (temp == 0) {
                pw.println(Math.abs(all.get(temp) - m));
            }
            else if (temp == all.size()) {
