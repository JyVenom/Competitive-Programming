...
//        List<List<Node>> adj = new ArrayList<>();
        ...
        DijkstrasShortestPathAdjacencyListWithDHeap dsp = new DijkstrasShortestPathAdjacencyListWithDHeap(n);
...
//                adj.get(i).add(new Node(Integer.parseInt(line[2 + j]) - 1, cost));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < size; j++) {
                dsp.addEdge(i, connections2.get(i).get(j), cost);
            }
        }
...
class DijkstrasShortestPathAdjacencyListWithDHeap {
    ...
    public int[] dijkstra(int start) {
        HashSet<Integer> temp = new HashSet<>(); //Jerry
