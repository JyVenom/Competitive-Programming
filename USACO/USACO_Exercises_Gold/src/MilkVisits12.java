...
public class MilkVisits12 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        dfs(friends, edges, path, visited, lasts, times, type, ans, 0);
    ...
    private static void dfs(HashMap<Integer, HashMap<Integer, ArrayList<int[]>>> friends, ArrayList<ArrayList<Integer>> edges, ArrayList<Integer> path, boolean[] visited, int[][] last, int[][] times, int[] type, int[] ans, int cur) {
        visited[cur] = true;
        int temp = path.size();
        path.add(cur);
        ...
        for (int next : edges.get(cur)) {
            if (!visited[next]) {
                dfs(friends, edges, path, visited, last, times, type, ans, next);
            }
        }
