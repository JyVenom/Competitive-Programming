...
public class CowNavigation2 {
    public static void main(String[] args) throws IOException {
        ...
        int a = helper(path, data, new boolean[n][n][4], 0, 0, 2, N, Integer.MAX_VALUE / 2, d, 2);
        path = new ArrayList<>();
        int b = helper(path, data, new boolean[n][n][4], 0, 0, 1, N, Integer.MAX_VALUE / 2, d, 1);
    ...
    private static int helper(ArrayList<Integer> path, boolean[][] data, boolean[][][] visited, int row, int col, int dir, int N, int min, Dijkstra d, int init) {
        if (row == N && col == N) {
            int n = N + 1;
            int[] temp = simulate(path, data, init == 2 ? 1 : 2, N);
            int sum = path.size();
            sum += d.dijkstra((temp[0] * n * 4) + (temp[1] * 4) + temp[2], n * n * 4);
        ...
        copy.add(0);
        if (dir == 0) {
            if (row > 0 && !data[row - 1][col] && !visited[row - 1][col][dir]) {
                min = helper(copy, data, visited, row - 1, col, dir, N, min, d, init);
            }
        }
        else if (dir == 1) {
            if (col < N && !data[row][col + 1] && !visited[row][col + 1][dir]) {
                min = helper(copy, data, visited, row, col + 1, dir, N, min, d, init);
            }
        }
        else if (dir == 2) {
            if (row < N && !data[row + 1][col] && !visited[row + 1][col][dir]) {
                min = helper(copy, data, visited, row + 1, col, dir, N, min, d, init);
            }
        }
        else {
            if (col > 0 && !data[row][col - 1] && !visited[row][col - 1][dir]) {
                min = helper(copy, data, visited, row, col - 1, dir, N, min, d, init);
            }
        }
        ...
        if (!visited[row][col][newDir]) {
            min = helper(copy, data, visited, row, col, newDir, N, min, d, init);
        }
