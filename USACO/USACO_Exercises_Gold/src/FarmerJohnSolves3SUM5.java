...
public class FarmerJohnSolves3SUM5 {
    public static void main(String[] args) throws IOException {
        ...
        HashMap<Integer, HashMap<Integer, Integer>> all = threeSum(nums);
//        for (int i = 0; i < q; i++) {
//            int[] range = ranges[i];
//
//            int sum = 0;
//            for (int min : all.keySet()) {
//                if (min < range[0] || min > range[1]) {
//                    continue;
//                }
//                for (int max : all.get(min).keySet()) {
//                    if (max <= range[1]) {
//                        sum += all.get(min).get(max);
//                    }
//                }
//            }
//            pw.println(sum);
//        }
    ...
    private static HashMap<Integer, HashMap<Integer, Integer>> threeSum(int[][] nums) {
                        ...
                        for (int l = k; l > j; l--) {
                            for (int m = j; m < l; m++) {
                                ...
                                if (!result.containsKey(min)) {
                                    result.put(min, new HashMap<>());
                                }
                                if (!result.get(min).containsKey(max)) {
                                    result.get(min).put(max, 1);
                                } else {
                                    result.get(min).replace(max, result.get(min).get(max) + 1);
                                }
                            }
                        }
                        break;
