...
public class MilkVisits30 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<Integer, HashMap<Integer, ArrayList<int[]>>> friends = new HashMap<>(n);
        for (int i = 0; i < m; i++) {
            int a = r.nextInt() - 1;
            int b = r.nextInt() - 1;
            int c = r.nextInt() - 1;
            ...
            if (!friends.containsKey(a)) {
                friends.put(a, new HashMap<>(n));
            }
            if (!friends.get(a).containsKey(b)) {
                friends.get(a).put(b, new ArrayList<>(m));
            }
            friends.get(a).get(b).add(new int[]{c, i});

            if (b != a) {
                if (!friends.containsKey(b)) {
                    friends.put(b, new HashMap<>(n));
                }
                if (!friends.get(b).containsKey(a)) {
                    friends.get(b).put(a, new ArrayList<>(m));
                }
