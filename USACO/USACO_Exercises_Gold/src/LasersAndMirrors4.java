...
public class LasersAndMirrors4 {
    public static void main(String[] args) throws IOException {
        ...
        pw.println(DFS(edges, new boolean[N2], new point2(0, 0)) - 1);
        pw.close();
    }

    private static int DFS(ArrayList<ArrayList<Integer>> edges, boolean[] visited, point2 cur) {
        if (cur.index == 1) {
            return cur.level;
        }
        ...
        for (int next : edges.get(cur.index)) {
            if (!visited[next]) {
                ...
                int temp = DFS(edges, visited, new point2(next, cur.level + 1));
                if (temp != 0) {
