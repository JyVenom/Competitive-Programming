...
public class LasersAndMirrors7 {
    public static void main(String[] args) throws IOException {
        ...
        rows2.add(yL);
        cols2.add(xL);
        rows.put(yL, new ArrayList<>());
        rows.get(yL).add(0);
        cols.put(xL, new ArrayList<>());
        cols.get(xL).add(0);
        if (!rows.containsKey(yB)) {
            rows2.add(yB);
            rows.put(yB, new ArrayList<>());
        }
        rows.get(yB).add(1);
        if (!cols.containsKey(xB)) {
            cols2.add(xB);
            cols.put(xB, new ArrayList<>());
        }
        cols.get(xB).add(1);
        for (int i = 0; i < N; i++) {
            ...
            if (!rows.containsKey(y)) {
                rows2.add(y);
                rows.put(y, new ArrayList<>());
            }
            rows.get(y).add(I);
            if (!cols.containsKey(x)) {
                cols2.add(x);
                cols.put(x, new ArrayList<>());
            }
            cols.get(x).add(I);
        }
        ...
        for (int row : rows2) {
            int size = rows.get(row).size() - 1;
            for (int i = 0; i < size; i++) {
                int cur = rows.get(row).get(i);
                for (int j = i + 1; j < rows.get(row).size(); j++) {
                    int next = rows.get(row).get(j);
        ...
        for (int col : cols2) {
            int size = cols.get(col).size() - 1;
            for (int i = 0; i < size; i++) {
                int cur = cols.get(col).get(i);
                for (int j = i + 1; j < cols.get(col).size(); j++) {
                    int next = cols.get(col).get(j);
                    edges.get(cur).add(next);
                    edges.get(next).add(cur);
                }
            }
