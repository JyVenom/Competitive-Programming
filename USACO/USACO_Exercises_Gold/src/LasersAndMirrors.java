...
public class LasersAndMirrors {
    public static void main(String[] args) throws IOException {
        ...
        rows.sort(Comparator.comparingInt(o -> o.x));
        rows.sort(Comparator.comparingInt(o -> o.y));
        cols.sort(Comparator.comparingInt(o -> o.y));
        cols.sort(Comparator.comparingInt(o -> o.x));
        int N2 = N + 2;
        int n = (N2) * 4;
        Dijkstra d = new Dijkstra((n) + 2);
        for (int i = 0; i < N2; i++) {
            int y = rows.get(i).y;
            int cur = rows.get(i).index * 4;
            for (int j = i + 1; j < N2 && rows.get(j).y == y; j++) {
                int next = rows.get(j).index * 4;
                d.addEdge(cur, next + 3, 1);
                d.addEdge(cur + 2, next + 3, 1);
                d.addEdge(next, cur + 1, 1);
                d.addEdge(next + 2, cur + 1, 1);
            }
        }
        for (int i = 0; i < N2; i++) {
            int x = cols.get(i).x;
            int cur = cols.get(i).index * 4;
            for (int j = i + 1; j < N2 && cols.get(j).x == x; j++) {
                int next = cols.get(j).index * 4;
                d.addEdge(cur + 1, next + 2, 1);
                d.addEdge(cur + 3, next + 2, 1);
                d.addEdge(next + 1, cur, 1);
                d.addEdge(next + 3, cur, 1);
            }
        }
        int n1 = n + 1;
        d.addEdge(n, 0, 0);
