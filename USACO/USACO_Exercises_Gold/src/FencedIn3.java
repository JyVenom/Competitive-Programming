...
//                dist.get(Math.min(cur, right)).put(Math.max(cur, right), rowH[i]);
//                dist.get(Math.min(cur, up)).put(Math.max(cur, up), colW[j]);
            }
        }
        for (int i = 0; i < n; i++) {
            int cur = m * N + i;
...
//            dist.get(Math.min(cur, right)).put(Math.max(cur, right), rowH[m]);
        }
        for (int i = 0; i < m; i++) {
            int cur = i * N + n;
            int up = (i + 1) * N + n;
            EagerPrimsAdjacencyList.addUndirectedEdge(g, cur, up, colW[n]);
//            dist.get(Math.min(cur, up)).put(Math.max(cur, up), colW[n]);
        }
    ...
    private static class EagerPrimsAdjacencyList {
