...
public class Moocast2 {
    public static void main(String[] args) throws IOException {
        ...
        Arrays.fill(minDist, 625000000);
        boolean[] visited = new boolean[n];
        MinIndexedDHeap<Double> ipq = new MinIndexedDHeap<>(edgeCount / n, n);
        ipq.insert(0, 0.0);
        while (!ipq.isEmpty()) {
            int cur = ipq.peekMinKeyIndex();
            double value = ipq.pollMinValue();
            visited[cur] = true;
            minDist[cur] = value;
            for (int i = 0; i < n; i++) {
                if (i == cur) {
                ...
                if (!visited[i]) {
                    if (!ipq.contains(i)) {
                        ipq.insert(i, dist[cur][i]);
                    } else {
                        ipq.decrease(i, dist[cur][i]);
                    }
                }
            }
        }
