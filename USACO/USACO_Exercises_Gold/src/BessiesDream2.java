...
public class BessiesDream2 {
    public static void main(String[] args) throws IOException {
        ...
        DijkstrasShortestPathAdjacencyListWithDHeapWithIntegers d = new DijkstrasShortestPathAdjacencyListWithDHeapWithIntegers(max);
        int N = n - 1;
        int M = m - 1;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (map[i][j] != 4) {
                    if (i > 0) {
                        if (map[i - 1][j] != 4 && map[i - 1][j] != 0) {
                            d.addEdge(i * n + j, (i - 1) * n + j, 1);
                        }
                        else if (map[i - 1][j] == 4) {
                            int[] next = move(map, N, M, 0, i, j);
    ...
    private static int[] move(int[][] map, int n, int m, int dir, int row, int col) {
        if (dir == 0) {
            if (row > 0) {
                if (map[row - 1][col] == 0) {
                    ...
                    return move(map, dir, n, m, row - 1, col);
                }
                else {
            ...
            if (col > 0) {
                if (map[row][col - 1] == 0) {
                    ...
                    return move(map, dir, n, m, row, col - 1);
                }
                else {
