...
//            hor[i] = Integer.parseInt(br.readLine());
        ...
        int size = N * M;
        ArrayList<HashMap<Integer, Integer>> dist = new ArrayList<>();
...
//        int[][] dist = new int[size][size];
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                dist.get(cur).put(right, rowH[i]);
                dist.get(right).put(cur, rowH[i]);
                dist.get(cur).put(up, colW[j]);
                dist.get(up).put(cur, colW[j]);
            }
        }
        for (int i = 0; i < n; i++) {
            int cur = m * N + i;
            ...
            dist.get(cur).put(right, rowH[m]);
            dist.get(right).put(cur, rowH[m]);
        }
        for (int i = 0; i < m; i++) {
                int cur = i * N + n;
                int up = (i + 1) * N + n;
                addEdge(graph, cur, up, colW[n]);
            dist.get(cur).put(up, colW[n]);
            dist.get(up).put(cur, colW[n]);
        }
    ...
    private static long mst(Graph graph, ArrayList<HashMap<Integer, Integer>> dist) {
