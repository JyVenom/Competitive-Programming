...
public class MilkVisits18 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        edges.get(0).add(0);
        dfs(friends, edges, path, lasts, type, ans, 0, -1, 1);
        System.out.println(System.currentTimeMillis() - start);
        start = System.currentTimeMillis();
    ...
    private static void dfs(HashMap<Integer, HashMap<Integer, ArrayList<int[]>>> friends, ArrayList<ArrayList<Integer>> edges, int[] path, int[][] last, int[] type, boolean[] ans, int cur, int prev, int depth) {
        path[depth] = cur;
        int[] temp = last[type[cur]].clone();
        last[type[cur]][0] = cur;
        last[type[cur]][1] = depth;
        ...
        for (int next : edges.get(cur)) {
            if (next != prev) {
                dfs(friends, edges, path, last, type, ans, next, cur, depth + 1);
            }
        }
