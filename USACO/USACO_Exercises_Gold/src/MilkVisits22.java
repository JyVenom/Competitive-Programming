...
public class MilkVisits22 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> friends = new HashMap<>();
        for (int i = 0; i < m; i++) {
            st = new StringTokenizer(br.readLine());
            int a = Integer.parseInt(st.nextToken()) - 1;
            int b = Integer.parseInt(st.nextToken()) - 1;
            ...
            if (!friends.get(a).containsKey(b)) {
                friends.get(a).put(b, new HashMap<>());
            }
            if (!friends.get(a).get(b).containsKey(c)) {
                friends.get(a).get(b).put(c, new ArrayList<>());
            }
            friends.get(a).get(b).get(c).add(i);
            ...
            if (a != b) {
                ...
                if (!friends.get(b).containsKey(a)) {
                    friends.get(b).put(a, new HashMap<>());
                }
                if (!friends.get(b).get(a).containsKey(c)) {
                    friends.get(b).get(a).put(c, new ArrayList<>());
                }
                friends.get(b).get(a).get(c).add(i);
            }
        }
        br.close();
    ...
    private static void dfs(HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> friends, ArrayList<ArrayList<Integer>> edges, ArrayList<Integer> path, int[][] last, int[][] times, int[] type, boolean[] ans, int cur, int parent) {
        int temp = path.size();
        path.add(cur);
        ...
        if (friends.containsKey(cur)) {
            for (int b : friends.get(cur).keySet()) {
                for (int c : friends.get(cur).get(b).keySet()) {

                    if (ans[friends.get(cur).get(b).get(c).get(0)]) {
                        continue;
                    }

                    if (cur == b) {
                        if (type[b] == c) {
                            setTrue(friends.get(cur).get(b).get(c), ans);
                        }
                    } else {
                        if (!(last[c][1] == 0)) {
                            int y = last[c][0];
    ...
    private static void setTrue(ArrayList<Integer> list, boolean[] ans) {
        for (int cur : list) {
            ans[cur] = true;
        }
    }
