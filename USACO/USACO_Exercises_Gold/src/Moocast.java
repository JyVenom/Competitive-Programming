...
public class Moocast {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            cows[i] = Arrays.stream(br.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        }

        int[][] dist = new int[n][n];
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                int curDist = dist(cows[i], cows[j]);
                dist[i][j] = curDist;
                dist[j][i] = curDist;
            }
        }
        ...
        minDist[0] = 0;
        while (!pq.isEmpty()) {
            Pair<Integer, Integer> info = pq.poll();
            int cur = info.right;
            visited[cur] = true;
            for (int i = 0; i < n; i++) {
                if (i == cur) {
    ...
    private static int dist(int[] a, int[] b) {
        double ac = Math.abs(b[1] - a[1]);
        double cb = Math.abs(a[1] - a[0]);

        return (int) Math.hypot(ac, cb);
    }

    static class Pair<L extends Comparable<L>, R> implements Comparable<BessiesDream12.Pair<L, R>> {
        public L left;
        public R right;
        ...
        public int compareTo(BessiesDream12.Pair<L, R> other) {
