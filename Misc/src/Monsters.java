...
public class Monsters {
    ...
    public static void main(String[] args) throws IOException {
        ...
        DFS(lab, new boolean[n][m], dist, start[0], start[1], 0, n - 1, m - 1);
    ...
    private static void DFS(boolean[][] lab, boolean[][] visited, int[][] dist, int row, int col, int cost, int N, int M) {
        ...
        if (!visited[row - 1][col] && lab[row - 1][col]) {
            path.add('U');
            DFS(lab, visited, dist, row - 1, col, cost + 1, N, M);
            if (done) {
                return;
            }
            path.remove(sz);
        ...
        if (!visited[row][col + 1] && lab[row][col + 1]) {
            path.add('R');
            DFS(lab, visited, dist, row, col + 1, cost + 1, N, M);
            if (done) {
                return;
            }
            path.remove(sz);
