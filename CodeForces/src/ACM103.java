...
public class ACM103 {
    public static void main(String[] args) throws IOException {
        ...
        int source = ir.nextInt() - 1;
        int dest = ir.nextInt() - 1;
        int n = ir.nextInt();
        int m = ir.nextInt();
        light[] lights = new light[n];
        for (int i = 0; i < n; i++) {
            lights[i] = new light(ir.nextLine().equals("B") ? 0 : 1, ir.nextInt(), ir.nextInt(), ir.nextInt());
        }
        ...
        ArrayList<Integer> path = d.reconstructPath(lights, source, dest);
    ...
    private static class light {
        int color, rem, blue, purple;

        public light(int color, int rem, int blue, int purple) {
            this.color = color;
            this.rem = rem;
            this.blue = blue;
            this.purple = purple;
    ...
    public static class DijkstraShortestPath {
        ...
        // Integer.MAX_VALUE.
        public int dijkstra(light[] lights, int start, int end) {
            ...
            while (!ipq.isEmpty()) {
                ...
                for (Edge edge : graph.get(nodeId)) {
                    ...
                    int newDist = findCost(lights, nodeId, edge.to, dist[nodeId]);
                    if (newDist != -1) {
        ...
        private int findCost(light[] lights, int a, int b, int time) {
            int curA, curB, remA, remB;
            if (time < lights[a].rem) {
                curA = lights[a].color;
                remA = lights[a].rem - time;
            } else {
                int tmp = time;
                tmp -= lights[a].rem;
                tmp = tmp % (lights[a].blue + lights[a].purple);
                if (tmp < lights[a].blue) {
                    curA = 0;
                    remA = lights[a].blue - tmp;
                } else {
                    curA = 1;
                    remA = lights[a].purple - (tmp - lights[a].blue);
                }
            }
            if (time < lights[b].rem) {
                curB = lights[b].color;
                remB = lights[b].rem - time;
            } else {
                int tmp = time;
                tmp -= lights[b].rem;
                tmp = tmp % (lights[b].blue + lights[b].purple);
                if (tmp < lights[b].blue) {
                    curB = 0;
                    remB = lights[b].blue - tmp;
                } else {
                    curB = 1;
                    remB = lights[b].purple - (tmp - lights[b].blue);
                }
            }
            ...
            if (curA == curB) {
                ...
                if (remA != remB) {
                    ...
                    curA = (curA == 0 ? 1 : 0);
                    if (curA == 1) {
                        if (lights[a].purple != lights[b].blue) {
                            return tmp + Math.min(lights[a].purple, lights[b].blue);
                        } else {
                            tmp += lights[a].purple;
                            if (lights[a].blue != lights[b].purple) {
                                return tmp + Math.min(lights[a].blue, lights[b].purple);
                            }
                        }
                    } else {
                        if (lights[a].blue != lights[b].purple) {
         ...
         */
        public ArrayList<Integer> reconstructPath(light[] lights, int start, int end) {
            ...
            int dist = dijkstra(lights, start, end);
