...
public class P2B3 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = N1; i >= 0; i--) {
            num[i][N][0] = num[i + 1][N][0] + findNum(2, matrix[i][N]);
            num[i][N][1] = num[i + 1][N][1] + findNum(5, matrix[i][N]);
            dp[i][N] = Math.min(num[i][N][0], num[i][N][1]);
            for (int j = N1; j >= 0; j--) {
                int tmp2 = findNum(2, matrix[i][j]);
                int tmp5 = findNum(5, matrix[i][j]);
                int right = Math.min(num[i][j + 1][0] + tmp2, num[i][j + 1][1] + tmp5);
                int down = Math.min(num[i + 1][j][0] + tmp2, num[i + 1][j][1] + tmp5);
                if (right < down) {
                    num[i][j][0] = num[i][j + 1][0] + tmp2;
                    num[i][j][1] = num[i][j + 1][1] + tmp5;
                    dp[i][j] = right;
                    dir[i][j] = true;
                } else {
                    num[i][j][0] = num[i + 1][j][0] + tmp2;
                    num[i][j][1] = num[i + 1][j][1] + tmp5;
                    dp[i][j] = down;
                }
            }
        }
