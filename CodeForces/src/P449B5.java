...
public class P449B5 {
    public static void main(String[] args) throws IOException {
        ...
        long[] train = new long[n];
        Arrays.fill(train, Integer.MAX_VALUE);
        HashSet<Integer> canRemove = new HashSet<>();
        int count = 0;
        for (int i = 0; i < k; i++) {
            int s = ir.nextInt() - 1;
            long y = ir.nextInt();
            if (train[s] != Integer.MAX_VALUE) {
                count++;
            }
            canRemove.add(s);
            if (y < train[s]) {
        ...
        d.setTrain(train);
        d.dijkstra(0, canRemove);

        pw.println(canRemove.size() + count);
        pw.close();
    }
    ...
    private static class DijkstraShortestPath {
        ...
        // Integer.MAX_VALUE.
        public void dijkstra(int start, HashSet<Integer> canRemove) {
            ...
            while (!ipq.isEmpty()) {
                ...
                if (train[nodeId] < minValue) {
                    dist[nodeId] = train[nodeId];
                    canRemove.remove(nodeId);
                }
