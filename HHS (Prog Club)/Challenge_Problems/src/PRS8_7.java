...
public class PRS8_7 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            sites.add(new website(br.readLine(), rs[i], i + 1));
        }
        ...
        sites.sort((o1, o2) -> {
            if (o1.name.length() < o2.name.length()) {
                for (int i = 0; i < o1.name.length(); i++) {
                    int dif = o1.name.charAt(i) - o2.name.charAt(i);
                    if (dif != 0) {
                        return dif;
                    }
                }
        ...
        for (int i = 0; i < q; i++) {
            pw.println(findAns(sites, br.readLine()));
        }

        pw.close();
    }

    private static int findAns(ArrayList<website> sites, String s) {
    ...
    private static int binSearch(ArrayList<website> arr, String key) {
    ...
    private static int compare(String s1, String s2) {
        if (s1.length() < s2.length()) {
            for (int i = 0; i < s1.length(); i++) {
                int dif = s1.charAt(i) - s2.charAt(i);
                if (dif != 0) {
                    return dif;
                }
            }
    ...
    private static class website {
        ...
        public website(String name, int val, int num) {
            this.name = name;
            this.val = val;
            this.num = num;
        }
