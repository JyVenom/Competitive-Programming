...
public class E {
    public static void main(String[] args) throws IOException {
        ...
        int L = ir.nextInt();
        int R = ir.nextInt();
        HashSet<Integer> blocked = new HashSet<>(M);
        for (int i = 0; i < M; i++) {
            blocked.add(ir.nextInt());
        }
        ir.close();

        ArrayDeque<state> q = new ArrayDeque<>();
        q.addLast(new state(1, 1L));
        int MAX = R - L;
        long prob = R - L + 1;
        ...
        while (!q.isEmpty()) {
            ...
            int end = cur.at + R;
            for (int i = cur.at + L; i <= end; i++) {
                if (i >= 1 && i <= N && !blocked.contains(i)) {
                    q.addLast(new state(i, cur.den * prob));
                }
            }
        }
